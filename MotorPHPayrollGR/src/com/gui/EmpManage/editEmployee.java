...
public class editEmployee extends JPanel {
    ...
    private void initComponents() {
        ...
        dateChooserBirthday = new JDateChooser(); dateChooserBirthday.setDateFormatString("d-MMM-yy");
        ...
        // read-only fields
        txtEmpNum.setEditable(false); txtEmpNum.setBackground(Color.LIGHT_GRAY);
        txtGrossSemiMonthlyRate.setEditable(false); txtGrossSemiMonthlyRate.setBackground(Color.LIGHT_GRAY);
        txtHourlyRate.setEditable(false); txtHourlyRate.setBackground(Color.LIGHT_GRAY);
        ...
        gbc.gridy = row;
        gbc.gridx = 0; formPanel.add(new JLabel("Employee #:"), gbc);
        gbc.gridx = 1; formPanel.add(txtEmpNum, gbc);
        gbc.gridx = 2; formPanel.add(new JLabel("Last Name:"), gbc);
        gbc.gridx = 3; formPanel.add(txtLastname, gbc);
        gbc.gridx = 4; formPanel.add(new JLabel("First Name:"), gbc);
        gbc.gridx = 5; formPanel.add(txtFirstname, gbc);

        // Row 1: Address, Birthday
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("Address:"), gbc);
        gbc.gridx = 1; gbc.gridwidth = 3; formPanel.add(txtAddress, gbc);
        gbc.gridwidth = 1;
        gbc.gridx = 4; formPanel.add(new JLabel("Birthday:"), gbc);
        gbc.gridx = 5; formPanel.add(dateChooserBirthday, gbc);

        // Row 2: SSS, Pag-Ibig, TIN, PhilHealth
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("SSS #:"), gbc);
        gbc.gridx = 1; formPanel.add(txtSSS, gbc);
        gbc.gridx = 2; formPanel.add(new JLabel("Pag-Ibig #:"), gbc);
        gbc.gridx = 3; formPanel.add(txtPagIbig, gbc);
        gbc.gridx = 4; formPanel.add(new JLabel("TIN #:"), gbc);
        gbc.gridx = 5; formPanel.add(txtTin, gbc);

        gbc.gridy = ++row;
        gbc.gridx = 4; formPanel.add(new JLabel("Philhealth #:"), gbc);
        gbc.gridx = 5; formPanel.add(txtPhilHealth, gbc);

        // Row 4: Status and Phone
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("Status:"), gbc);
        gbc.gridx = 1; formPanel.add(cmbStatus, gbc);
        gbc.gridx = 2; formPanel.add(new JLabel("Phone #:"), gbc);
        gbc.gridx = 3; formPanel.add(txtPhoneNumber, gbc);

        // Row 5: Supervisor and Position
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("Immediate Supervisor:"), gbc);
        gbc.gridx = 1; formPanel.add(cmbSupervisor, gbc);
        gbc.gridx = 2; formPanel.add(new JLabel("Position:"), gbc);
        gbc.gridx = 3; formPanel.add(cmbPosition, gbc);

        // Row 6: Basic Salary, Rice Subsidy, Phone Allowance
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("Basic Salary:"), gbc);
        gbc.gridx = 1; formPanel.add(txtBasicSalary, gbc);
        gbc.gridx = 2; formPanel.add(new JLabel("Rice Subsidy:"), gbc);
        gbc.gridx = 3; formPanel.add(txtRiceSubsidy, gbc);
        gbc.gridx = 4; formPanel.add(new JLabel("Phone Allowance:"), gbc);
        gbc.gridx = 5; formPanel.add(txtPhoneAllowance, gbc);

        // Row 7: Gross Semi-Monthly Rate, Clothing Allowance
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("Gross Semi-Monthly Rate:"), gbc);
        gbc.gridx = 1; formPanel.add(txtGrossSemiMonthlyRate, gbc);
        gbc.gridx = 2; formPanel.add(new JLabel("Clothing Allowance:"), gbc);
        gbc.gridx = 3; formPanel.add(txtClothingAllowance, gbc);

        // Row 8: Hourly Rate
        gbc.gridy = ++row;
        gbc.gridx = 0; formPanel.add(new JLabel("Hourly Rate:"), gbc);
        gbc.gridx = 1; formPanel.add(txtHourlyRate, gbc);
    ...
    // 3) Append to EmployeeData.csv
       try (CSVWriter w = new CSVWriter(
        ...
        if (!dtrDir.exists()) dtrDir.mkdirs();
        ...
        try (CSVReader reader = new CSVReader(new FileReader("src/com/csv/EmployeeData.csv"))) {
            ...
            while ((row = reader.readNext()) != null) {
                if (row.length > 0 && empID.equals(row[0].trim())) {
                    ...
                    if (row.length > 1) txtFirstname.setText(row[1].trim());
                    if (row.length > 2) txtLastname .setText(row[2].trim());
                    ...
                    if (row.length > 4)  txtHourlyRate            .setText(row[4].trim());
                    if (row.length > 5)  txtRiceSubsidy           .setText(row[5].trim());
                    if (row.length > 6)  txtPhoneAllowance        .setText(row[6].trim());
                    if (row.length > 7)  txtClothingAllowance     .setText(row[7].trim());

                    // Status (idx 8), Position (9)
                    if (row.length > 8)  cmbStatus   .setSelectedItem(row[8].trim());
                    if (row.length > 9)  cmbPosition .setSelectedItem(row[9].trim());

                    // BasicSalary (10), Phone# (11)
                    if (row.length > 10) txtBasicSalary           .setText(row[10].trim());
                    if (row.length > 11) txtPhoneNumber           .setText(row[11].trim());

                    // SSS (12), PhilHealth (13), TIN (14), PagIbig (15)
                    if (row.length > 12) txtSSS                   .setText(row[12].trim());
                    if (row.length > 13) txtPhilHealth            .setText(row[13].trim());
                    if (row.length > 14) txtTin                   .setText(row[14].trim());
                    if (row.length > 15) txtPagIbig               .setText(row[15].trim());

                    // Supervisor (16), GrossSemi (17), Address (18)
                    if (row.length > 16) cmbSupervisor            .setSelectedItem(row[16].trim());
                    if (row.length > 17) txtGrossSemiMonthlyRate  .setText(row[17].trim());
                    if (row.length > 18) txtAddress               .setText(row[18].trim());
    ...
    private void calculateRates() {
        try {
            ...
            txtHourlyRate.setText(""); txtGrossSemiMonthlyRate.setText("");
        }
    }
    ...
    private void clearFields() {
        txtFirstname.setText(""); txtLastname.setText(""); dateChooserBirthday.setDate(null);
        txtAddress.setText(""); txtPhoneNumber.setText(""); txtSSS.setText(""); txtPhilHealth.setText("");
        txtTin.setText(""); txtPagIbig.setText(""); cmbStatus.setSelectedIndex(0); cmbPosition.setSelectedIndex(0);
        cmbSupervisor.setSelectedIndex(0); txtBasicSalary.setText(""); txtRiceSubsidy.setText("");
        txtPhoneAllowance.setText(""); txtClothingAllowance.setText(""); txtGrossSemiMonthlyRate.setText(""); txtHourlyRate.setText("");
    }
}
